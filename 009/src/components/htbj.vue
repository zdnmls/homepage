    ...
    export default {
        ...
        methods: {
            show (index) {
                this.$Modal.info({
                    title: '笔记详情',
                    content: `笔记：${this.data6[index].name}<br>内容：${this.data6[index].content}<br>浏览：${this.data6[index].pages}&nbsp;&nbsp;；&nbsp;&nbsp;关键字：${this.data6[index].card}<br>地址：${this.data6[index].times}`
                })
            },
            remove (row) {
                // this.data6.splice(index, 1);
                let ids = row.id;
                fetch('/api/notes_del?id='+ids).then(r=>r.json()).then(r=>this.data6 =r)
            },
            page1(r){
                this.pageNumber=r
            }
        ...
        components:{Htmb},
        mounted(){
            fetch('/api/notes').then(r=>r.json()).then(r=>this.data6 =r)
        }
    }
